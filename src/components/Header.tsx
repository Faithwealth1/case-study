...
export const Header: React.FC = () => {
  ...
  return (
    <header className="flex w-[1280px] justify-between items-center backdrop-blur-2xl z-[1000] bg-[rgba(8,8,8,0.90)] px-8 py-4 rounded-[30px] mx-auto mt-[60px] mb-10 max-md:w-[90%] max-md:px-6 max-md:py-3 max-sm:w-[95%] max-sm:px-5 max-sm:py-2.5">
      <div className="flex w-[100px] h-[22px] justify-center items-center shrink-0 relative">
        <img
          src="vontech logo.png"
          alt="VonTech Logo"
        ...
        <nav className="flex items-center gap-9">
          {navigationItems.map((item) => (
            <div key={item.label} className="flex items-center gap-[5px]">
              <a
                href={item.href}
                className="text-white text-xl font-medium leading-[30px] cursor-pointer hover:text-[#FFE21B] transition-colors"
              >
                {item.label}
              </a>
              {item.hasDropdown && (
